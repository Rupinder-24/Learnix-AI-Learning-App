...
if (!process.env.GEMINI_API_KEY) {
    ...
    console.error('FATAL ERROR: GEMINI_API KEY is not set in the environment variables.');
...
export const generateFlashcards = async (text, count = 10) => {
    ...
    try {
        ...
        console.error('Gemimi API error', error);
...
export const generateQuiz = async (text, numQuestions = 5) => {
    ...
    try {
        ...
        for (const block of questionBlocks) {
            ...
            if (question && options.length === 4 && options.includes(correctAnswer)) {
