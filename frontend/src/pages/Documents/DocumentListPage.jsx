...
const DocumentListPage = () => {
  ...
  // State or upload modal
  const[isUploadModalOpen,setIsUploadModalOpen]=useState(false);
  const[uploadFile,setUploadFile]=useState(null);
  const[uploadTitle,setUploadTitle]=useState("");
  const[uploading,setUploading]=useState(false);

  // state or delete comfimation madal
  ...
  const fetchDocuments=async()=>{
    try{
      ...
      toast.error("Failed to fetch Doocuments")
      console.error(error);
    }finally{
      setLoading(false);
    }
          ...
          <p className='text-sm text-slate-600 mb-6'>
            ...
            <div className='flex gap-3'>
              ...
              >
                ...
                disabled={deleting}
                className='flex-1 h-11 px-4 bg-linear-to-r from-red-500 to-red-600  rounded-xl text-white text-sm font-semibold hover:from-red-500 hover:to-red-700 shadow-lg shadow-red-500/25 transition-all duration-200 disabled:opacity-50 active:scale-[0.98]'
                >
                  {deleting?(
                    <span className='flex items-center justify-center gap-2'>
                      <div className='w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin'/>
